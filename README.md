...
This project is an E-commerce application developed as part of the MVP phase requirements for the TAP program. It provides a backend API that supports
user authentication, product management, and simulated payment processing. The application is designed with scalability, modularity, and security in
mind, using JWT for authentication, PostgreSQL for the database, and Redis for caching.
...
The application includes a simulatePaymentProcessing controller that simulates success or failure for payment processing. The success rate can be
configured for testing purposes. Transactions are saved in the database for historical data analysis.

### Error Handling

Error handling has been implemented across all controllers and routes, with meaningful status codes and error messages. Errors are logged for
debugging purposes.
...
Use tools like Apache JMeter to perform load testing on the APIs. The results used to analyze the app to optimize performance and scalability.
